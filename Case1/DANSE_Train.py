...
model_weights_path = 'model/DANSE6.pt'
...
model.load_state_dict(torch.load('model/DANSE5.pt'))
...
for epoch in range(num_epochs):
    ...
    for batch_index, (data, labels) in enumerate(Train_loader):
        data = data.view(-1, 3, L).transpose(2, 1).to(device)
        B, _, _ = data.shape
        ...
        z_pred, H = h(x_pred)
        P_zz = H @ P_pred @ H.transpose(1, 2) + torch.diag(torch.tensor([100, 100, 100], device=device)).unsqueeze(0).repeat(B*200, 1, 1) # [B*200, 3, 3]
        labels = labels.reshape(-1, 3) # [B*200, 3]
        residual = labels - z_pred # [B*200, 3]
        residual = residual.unsqueeze(2)  # [B*200, 3, 1]
