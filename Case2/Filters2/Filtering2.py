...
X_TRUE = np.loadtxt("data2/Test_true_filter.txt", dtype='float64')
Z_TRUE = np.loadtxt("data2/Test_data.txt", dtype='float64')
...
def non_equ(x, z):
    ...
    for i in range(L_warm):
        if i == 0:
            Z.append((h(x) - z[:, i]) / np.array([100., 10.]))
        else:
            x_pred = f(x, [0, T * i])
            z_pred = h(x_pred)
            Z.append((z_pred - z[:, i]) / np.array([100., 10.]))
    Z = np.hstack(Z)
    return Z
...
def jac_non_equ(x, z):
    ...
    for i in range(L_warm):
        if i == 0:
            H_total.append(J_h(x) / np.array([[100, 100, 100], [10, 10, 10]]))
        else:
            x_pred, Phi = f_EKF(x, [0, T * i])
            H = J_h(x_pred)
            H_total.append((H / np.array([[100, 100, 100], [10, 10, 10]])) @ Phi )
...
print('t_PF_2', t_PF_2)
print('t_CLRB', t_PCRB)
# np.savetxt('RMSE_UKF_2.txt', RMSE_UKF)
# np.savetxt('RMSE_EKF_2.txt', RMSE_EKF)
# np.savetxt('RMSE_PF_1_2.txt', RMSE_PF_1)
# np.savetxt('RMSE_PF_2_2.txt', RMSE_PF_2)
# np.savetxt('RMSE_CLRB_2.txt', RMSE_PCRB)
