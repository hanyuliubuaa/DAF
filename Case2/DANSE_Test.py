...
model_weights_path = 'model/DANSE2_725.pt'
...
Z_TRUE = torch.tensor(np.loadtxt("data2/Test_data.txt", dtype='float64'), device=device).view(-1, D_in, L).transpose(2, 1)
label = torch.tensor(np.loadtxt("data2/Test_label.txt", dtype='float64'), device=device).view(-1, D_in, L-L_warm).transpose(2, 1)
...
with torch.no_grad():
    ...
    z_pred, H = h(x_pred)
    P_zz = H @ P_pred @ H.transpose(1, 2) + torch.diag(torch.tensor([10000, 100], device=device)).unsqueeze(0).repeat(500*200, 1, 1) # [B*200, 2, 2]
